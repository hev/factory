...
* `docker compose version` reports the plugin (v2 or later). `lab up ci-runners` on the mini restarts the three layer-pro runners and `gh api repos/hev/layer-pro/actions/runners` reports all three `online` within two minutes.
