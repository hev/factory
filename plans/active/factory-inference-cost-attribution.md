...
Source handoff is ready in https://github.com/hev/factory/pull/18 (7a5a9ec) and https://github.com/hev/kit/pull/32 (7b2e5b5). Source tests, isolated replay and local real-data browser checks pass against the amended criteria. The [acceptance matrix](https://github.com/hev/kit/blob/7b2e5b5/docs/cost-attribution-evidence.md) distinguishes that evidence from deployed acceptance; all six original steps remain open below.
...
- **Step 6 and backfill:** after the operator merges the combined contract and authorizes rollout/restart, verify new beat/report dollar fields, including pending and incomplete/null states; never rewrite historical beat lines. The supported `scripts/costs/eval_rows.py` → `hev eval put` adapter now passes duplicate-free isolated replay. Verify hosted-store and embedding semantics, then run the authorized production backfill; isolated HTTP-store evidence does not satisfy this tail.

Serving-host checks retain the recorded-history boundary and role `other` in the subscription denominator; unavailable older history is not a new blocker. The same-week provider-UI comparison remains independent of the usage API. Shipping proof stays bounded by retained records, explicit references, configured deploy jobs and ancestry, without claiming artifact or missing-history completeness.

Preview workflow and cache rollout/one-week measurements remain in [their](preview-screenshot-verification.md) [active plans](worker-shared-build-caches.md). Private session audits and billing inputs remain machine-local.
