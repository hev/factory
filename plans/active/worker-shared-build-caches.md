...
3. **Bound the shared cache.** A weekly `cargo sweep --maxsize 40G` (or `--time 7`) over each shared target dir, in the same lab-declared job as <issue id="d7732a3c-1ef6-4e80-9aca-30df3c26b641" href="https://linear.app/hevmind/issue/FAC-13/the-mini-comes-back-from-a-hard-restart-unattended">FAC-13</issue> step 5's docker prune. *Accept:* idle targets are evicted weekly above 40 GiB; active targets are retained and reported. The job's log shows what it removed.
...
- Steps 3–4: https://linear.app/hevmind/issue/FAC-24 accepted weekly idle eviction above 40 GiB with active targets retained and reported on 2026-09-09; https://github.com/hev/lab/pull/6 is merged. Rollout still respects the combined contract gate; verify the installed policy and doctor after it clears.
