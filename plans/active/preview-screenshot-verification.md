...
Step 8's original https://github.com/hev/factory/pull/15 closed unmerged after its exact correction was preserved in the combined draft https://github.com/hev/factory/pull/18. The contract gate remains open.

Step 9 has local doctor, domain-refusal, concurrent-session isolation and explicit-cleanup evidence. Remaining acceptance is the next approved in-scope UI change with configured preview domains: deployed worker captures, independent gaffer before/after, uploaded preview-first review, live worker/reaper isolation and expiry, and the substantiated ten-minute no-preview fallback. Local fixtures do not satisfy those deployed workflow checks.
