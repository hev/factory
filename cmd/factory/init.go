...
func runInit(root string, args []string) error {
	...
	runtime := fs.String("runtime", "sessions", "sessions (foreman and assigned gaffers), one-shot or resident (legacy)")
...
func buildInstance(root string, a answers) (*instance, error) {
	...
	switch runtime {
	case "sessions", "resident", "one-shot":
	default:
		return nil, fmt.Errorf("--runtime %q is not sessions, resident or one-shot", runtime)
	}
...
const initUsage = `factory init — write one factory's config
  ...
  --runtime       sessions (default), one-shot or resident (legacy)
