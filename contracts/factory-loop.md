...
3. **Dispatch.** **Goals are wide, workers are smart**: a goal hands a worker
   ...
   **(a) if stuck or blocked, say so** — say it on the wire with
   ...
   bar in that file, and never as a separate approval; **(d) stop at the pull
   request** — once it is open and the `pr` state is posted, the worker's
   job is done. It does not poll CI checks and it does not merge, `gh pr
   merge --squash` included, even once every check is green: merging is the
   gaffer's decision, gated by "Graduated self-merge" below, never the
   worker's to take on the strength of a clean self-review; **(e) say so on
